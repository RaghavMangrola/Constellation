...
![Audio Constellation Demo](demo-preview.png)
...
4. **AudioVisualizerApp.swift** - Main app entry point and UI coordination
   - SwiftUI-based interface with permission handling
...
3. **Add Microphone Permission**:
   Add this key to your Info.plist or in Xcode project settings:
   ```xml
   <key>NSMicrophoneUsageDescription</key>
   <string>This app uses the microphone to create real-time audio visualizations.</string>
   ```
...
- **Real-time Processing**: ~23ms latency (1024 samples at 44.1kHz)
...
### Audio Processing
```swift
private let bufferSize: Int = 4096        // FFT size
private let sampleRate: Double = 48000.0  // Sample rate
```

### Peak Detection
```swift
private let minPeakHeight: Float = -60.0  // dB threshold
private let minPeakDistance: Int = 5      // Min bins between peaks
private let maxPeaksPerFrame: Int = 10    // Peaks per analysis
```

### Visual Effects
```swift
private let peakFadeTime: Double = 3.0    // Fade duration
private let maxStars = 1000               // Max rendered stars
```
...
1. **No microphone permission**: Check Settings → Privacy → Microphone
2. **No stars appearing**: Ensure audio is loud enough (>-60dB)
3. **Performance issues**: Reduce `maxStars` or `bufferSize`
